...
class Combat {
public : 
    enum class EtatCombat { //les diffs états du combat à afficher 
        Intro, //on papotte
        ChoixJoueur, //on demande ce que veux le joueur
        ChangementPokemon, //il change de poke
        AttaqueJoueur, //il attaque
        AttaqueAdversaire, //tour adveraire, je rajouterais son changement de poke plus tard
        KO_Joueur, //pokemon du joueurs est ko
        KO_Adversaire, //poke du adv est ko
...
public:
    ...
    void bagarre() {
        ...
        EtatCombat etatActuel = EtatCombat::Intro; 
        bool JoueurAchoisis = false;
        ...
        while (!joueur.getVaincu() && !adversaire.getVaincu()) {
            ...
            while (true) {
                ...
                if (choix == 1) {
                    JoueurAttaque = true; //le joueur décide d'attaquer ce tour
                    break;
                }
...
//pas besoin de fdelete les pointeurs par ce qu'ils pointent vers des objets deja existant, j'ai pas fais de new trucs
